...
export const Listing = ({ listItems, setListItems }: ListingProps): ReactElement => {
    const concluidas = listItems.filter((c) => c.status === "finished");

    const CompleteUncompleteTask = (index: number) => {
    ...
    const DeleteTask = (index: number) => {
        const updatedList = listItems.filter((_, itemIndex) => itemIndex !== index);
        setListItems(updatedList);
    };
    ...
    return (
        <div className={styles.mainDiv}>
            <div className={styles.statusRow}>
                <h3 className={`${styles.criadas} ${styles.mainbase}`}>
                    Tarefas criadas <span>{listItems.length}</span>
                </h3>
                <h3 className={`${styles.concluidas} ${styles.mainbase}`}>
                    Concluídas <span>{concluidas.length > 0 ? (`${concluidas.length} de ${listItems.length}`) : ("0")}</span>
                </h3>
            </div>
            ...
            {listItems.length > 0
                ?
                (
                    <div className={`${styles.content} ${styles.haveContent}`}>
                        {listItems.map((item, index) => {
                            const status = item.status === "active" ? styles.created : styles.finished;
                            return (
                                <div key={index} className={`${styles.listItem} ${status}`}>
                                    <div
                                        onClick={() => { CompleteUncompleteTask(index) }}
                                    />
                                    <p>
                                        {item.name}
                                    </p>
                                    <svg onClick={() => DeleteTask(index)} viewBox="0 0 13 14" xmlns="http://www.w3.org/2000/svg" className={styles.svg}>
                ...
                :
                (
                    <div className={`${styles.content} ${styles.noContent}`}>
